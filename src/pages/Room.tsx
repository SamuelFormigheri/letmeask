...
export function Room(){
    ...
    const {title, questions, handleCreateNewQuestion: AddNewQuestion, handleLikeQuestion } = useRoom(roomId);

    const handleCreateNewQuestion = async (e: FormEvent) => {
        e.preventDefault();

        if(!questionTextareaRef.current || questionTextareaRef.current.value.trim() === "" || !user)
            return;

        AddNewQuestion(questionTextareaRef.current.value);
